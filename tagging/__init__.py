...
def register(model, tag_descriptor_attr='tags',
    ...
    if model in registry:
        raise AlreadyRegistered("The model '%s' has already been registered." % model._meta.object_name)

    if hasattr(model, tag_descriptor_attr):
        raise AttributeError("'%s' already has an attribute '%s'. You must provide a custom tag_descriptor_attr to register." % (
